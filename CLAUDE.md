...
再生成後は差分を確認し、再生成の出力にpage dirの無いslug（コメントアウト済みエントリを生成器が拾う既知事象。過去に2件あり#330で削除済みだが、生成器は未対応のため新たなコメントアウトで再発し得る）が混入していないことを確認する。混入していれば当該slugを除去してからコミットする。
...
### セッション開始時
- .ai-team/DECISIONS.md の末尾数セクション（直近の決定）と .ai-team/QUALITY_RULES.md を確認してから作業する

### git運用
- コミットは対象ファイルの明示add のみ（git add -A 禁止）
- 機能PRは「リンGO（検証担当）→江田GO（最終承認）」の後にのみマージする。マージ指示文冒頭の「リンGO済み・」を確認する
- docs（.ai-team/配下）はmainへ直コミット・ff・逐語保存（発注文の内容を一字も変えない）
- 再フォーマット禁止（変更行のみの差分にする）。EOLはHEAD blobを基準にし、編集ツールがCRLFを混入させた場合はLFへ正規化してから差分を確認する。編集はHEAD blobを唯一の入力として組み立てる

### データ規律
- 数値は発注のDATA_ACTUALまたはリポジトリ内の検証済み値のみ。創作ゼロ
- 再エネ賦課金は4.18円/kWhのみ（check:surcharge が守る）
- 4区分単価の出典は「電力・ガス取引監視等委員会「電力取引報」から算出」に統一。委員会のURLは egc.meti.go.jp（旧emscは使用禁止）
- 未公表の値は「公表待ち」として扱い断定しない

### 表現規律（詳細は QUALITY_RULES.md）
- 誇大語（完全ガイド等の完全性主張・決定版・究極）禁止。「徹底◯◯」は姿勢表明として許容
- 結果を約束する断定（確実に◯◯できる等）・相対時間表現（残り約◯日・締切間近）禁止
- 出典のない相対比較（全国平均より◯円安い等）禁止
