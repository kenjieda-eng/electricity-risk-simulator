...
import type { CommercialIndustryArticle } from "../../lib/industryCommercialArticles";

type CommercialIndustryArticlePageProps = {
  article: CommercialIndustryArticle;
};
...
function CommercialIndustryVisuals({ article }: CommercialIndustryArticlePageProps) {
...
export default function CommercialIndustryArticlePage({
  article,
}: CommercialIndustryArticlePageProps) {
  const middleCategory = getIndustryMiddleCategory("commercial");
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-[1600px] bg-white px-4 py-8 text-slate-800 sm:px-6 lg:px-8">
      ...
      <header className="mt-4 rounded-xl border border-sky-200 bg-sky-50 p-6">
        <p className="text-sm font-semibold text-sky-800">商業系 / {article.name}</p>
        ...
        <CommercialIndustryVisuals article={article} />
      </header>
      ...
      <section className="mt-6 space-y-6">
        ...
        {relatedIndustryLinks.length ? (
          <RelatedLinks
            heading="商業系の関連業種"
            intro="近い業態もあわせて見ると、設備構成や契約の考え方の違いを整理しやすくなります。"
