...
type ManufacturingIndustryArticlePageProps = {
  article: ManufacturingIndustryArticle;
};

function ManufacturingIndustryChartBlock({ article }: ManufacturingIndustryArticlePageProps) {
...
export default function ManufacturingIndustryArticlePage({
  article,
}: ManufacturingIndustryArticlePageProps) {
  const middleCategory = getIndustryMiddleCategory("manufacturing");
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-[1600px] bg-white px-4 py-8 text-slate-800 sm:px-6 lg:px-8">
      ...
      <header className="mt-4 rounded-xl border border-sky-200 bg-sky-50 p-6">
        <p className="text-sm font-semibold text-sky-800">製造系 / {article.name}</p>
      ...
      <section className="mt-6 space-y-6">
        {article.sections.map((section) => {
          ...
          return (
            ...
            >
              ...
              {showChartHere ? <ManufacturingIndustryChartBlock article={article} /> : null}
              {section.bullets ? (
                <ul className="mt-3 list-disc space-y-2 pl-5 text-sm leading-7 text-slate-700 sm:text-base">
        ...
        {relatedIndustryLinks.length ? (
          <RelatedLinks
            heading="製造系の関連業種"
            intro="近い工程や負荷の型を持つ業種もあわせて見ると、契約や設備の見直しの比較がしやすくなります。"
