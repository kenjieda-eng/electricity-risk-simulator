...
import type { CommercialIndustryVisual } from "../../lib/industryCommercialArticles";
import HotelLeisureDiagram, { type HotelLeisureMidDiagram } from "./HotelLeisureDiagram";

export type HotelLeisureIndustrySection = {
  heading: string;
  paragraphs?: string[];
  diagram?: HotelLeisureMidDiagram;
  paragraphsAfterDiagram?: string[];
  bullets?: string[];
...
export type HotelLeisureIndustryArticle = {
  slug: string;
  name: string;
  ...
  learnPoints?: string[];
  keywords?: string[];
  sections: HotelLeisureIndustrySection[];
  relatedIndustrySlugs: string[];
  ctaDescription: string;
  visuals?: CommercialIndustryVisual;
  flowDiagram?: {
    heading: string;
    insertAfterSectionHeading: string;
    steps: { title: string; description: string }[];
  };
};

type Props = {
  article: HotelLeisureIndustryArticle;
};
...
function getDefaultLearnPoints(articleName: string) {
  return [
    ...
    `${articleName}で考えやすい見直しの方向性`,
  ];
}

function HeaderVisuals({ visuals }: { visuals: CommercialIndustryVisual }) {
  return (
    <>
...
function renderSectionBody(section: HotelLeisureIndustrySection) {
  return (
    <>
      {section.paragraphs?.map((paragraph, i) => (
        <p key={`${section.heading}-p-${i}`} className="mt-3 text-sm leading-7 text-slate-700 sm:text-base">
          {paragraph}
        </p>
      ))}
      {section.diagram ? <HotelLeisureDiagram spec={section.diagram} /> : null}
      {section.paragraphsAfterDiagram?.map((paragraph, i) => (
        <p key={`${section.heading}-pad-${i}`} className="mt-3 text-sm leading-7 text-slate-700 sm:text-base">
          {paragraph}
        </p>
      ))}
      {section.infoBoxes?.length ? (
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          {section.infoBoxes.map((box) => (
            <InfoBox key={box.title} title={box.title}>
              {box.body}
            </InfoBox>
          ))}
        </div>
      ...
      {section.paragraphsAfterBullets?.map((paragraph, i) => (
        <p key={`${section.heading}-pab-${i}`} className="mt-3 text-sm leading-7 text-slate-700 sm:text-base">
          {paragraph}
        </p>
      ))}
          ...
          {section.subsections.map((subsection) => (
            <article key={subsection.heading} className="rounded-xl border border-slate-200 bg-slate-50 p-4">
              ...
              {subsection.paragraphs?.map((paragraph, j) => (
                <p key={`${subsection.heading}-${j}`} className="mt-2 text-sm leading-7 text-slate-700 sm:text-base">
                  {paragraph}
                </p>
              ))}
              {subsection.bullets ? (
...
export default function HotelLeisureIndustryArticlePage({ article }: Props) {
  const middleCategory = getIndustryMiddleCategory("hotel-leisure");
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-[1600px] bg-white px-4 py-8 text-slate-800 sm:px-6 lg:px-8">
      ...
      <header className="mt-4 rounded-xl border border-sky-200 bg-sky-50 p-6">
        ...
        {article.visuals ? <HeaderVisuals visuals={article.visuals} /> : null}
      </header>
      ...
      <section className="mt-6 space-y-6">
        {article.sections.map((section) => {
          ...
          return (
            ...
            >
              ...
              {article.flowDiagram &&
              article.flowDiagram.insertAfterSectionHeading === section.heading ? (
                <div className="mt-4">
                  <FlowDiagram heading={article.flowDiagram.heading} steps={article.flowDiagram.steps} />
                </div>
              ) : null}
            </section>
          );
        ...
        {relatedIndustryLinks.length ? (
          <RelatedLinks
            ...
            intro="近い業態もあわせて見ると、負荷の出方や契約の考え方の違いを整理しやすくなります。"
