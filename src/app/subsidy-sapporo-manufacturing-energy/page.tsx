...
const keyPoints = [
  {
    name: "【要件】更新でエネルギー消費量を年率10%以上低減（自家消費型発電関連は5%以上）",
    role: "札幌市／★削減率が申請の前提条件",
    detail:
      "本制度では、設備の更新によってエネルギー消費量を年率10%以上低減することが求められます。対象となる設備としてLED照明・冷凍冷蔵設備・高効率空調・変圧器・生産設備等が挙げられており、これらの更新によって年率10%以上の低減を達成する必要があります。一方、自家消費型の発電に関連する設備については、5%以上の低減で要件を満たすとされています。この削減率は申請の前提条件であり、補助率3/4という高い水準の裏返しとして、相応の削減成果が求められる設計と読めます。年率10%以上という水準は、単一の小型機器の入れ替えでは届かない可能性があり、複数設備の同時更新や消費の大きい設備への集中投資が現実的な選択肢になります。削減率の算定方法・基準期間の取り方は公募要領で定まるため、自社の更新計画が要件を満たすかは最新の公式情報で必ず要確認としてください（出典: 札幌市／2026年度時点・要件確認必須）。",
  },
  {
...
const faqItems = [
  {
    question: "どのくらいの省エネ効果を達成する必要がありますか？",
    answer:
      "設備の更新によってエネルギー消費量を年率10%以上低減することが求められます。対象となる設備としてLED照明・冷凍冷蔵設備・高効率空調・変圧器・生産設備等が挙げられており、これらの更新によって年率10%以上の低減を達成する必要があります。一方、自家消費型の発電に関連する設備については、5%以上の低減で要件を満たすとされています。補助率3/4（共同受電設備を有する事業協同組合は4/5）という高い水準の裏返しとして、相応の削減成果が求められる設計といえます。年率10%以上という水準は単一の小型機器の入れ替えでは届かない可能性があるため、複数設備の同時更新や消費の大きい設備への集中投資を検討する価値があります。予算は3億円で、受付は2026年7月6日〜9月30日17時必着の先着です。削減率の算定方法・基準期間の取り方は公募要領で定まるため、最新の公式情報で必ずご確認ください。",
  },
  {
