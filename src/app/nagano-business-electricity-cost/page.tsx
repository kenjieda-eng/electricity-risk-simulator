...
const pageDescription =
  "長野県の法人電気代の相場・平均を、契約区分別の全国確定単価（高圧22.78円/kWh等）を基準に読む手順で整理。県内に中部電力エリアと東京電力エリアの境界が走るため、事業所ごとに供給エリアを確認したうえで電力会社を比較する5観点、諏訪の精密機械・軽井沢の観光業・果樹冷蔵の負荷特性を解説します。";
...
const electricSituation = [
  ...
  },
  {
    ...
    detail:
      "県内の標高差が大きい（200〜2,500m）。長野市・松本市は内陸性気候で寒暖差大。軽井沢・志賀高原・蓼科・白馬は標高1,000m超の高原気候で年間平均気温は10℃以下、冬季最低気温-20℃級。暖房度日3,500〜4,500の地域も多い。",
  },
  {
...
const utilitiesList = [
  {
    ...
    detail:
      "県内シェア最大（中部電力エリア部分）。高圧・特別高圧の標準メニューは『業務用季節別時間帯別電力』『業務用高圧電力』など。2023年6月の規制料金改定で家庭向け4.4%値上げ。法人向けも同等の改定を実施。",
  },
  {
...
const priceBenchmark = [
  {
    ...
    detail:
      "中部電力ミライズ『業務用高圧電力』の電力量料金は17〜21円/kWh（夏季・その他季・時間帯により変動）。燃料費調整額（2024〜2025年は+3.0〜+4.0円/kWh）と再エネ賦課金（2025年度3.98円/kWh）を加味すると実質単価は24〜29円/kWhレンジ。全国的な水準との差は契約区分・使用量・負荷率により変わります。",
  },
  {
...
const costFactors = [
  ...
  },
  {
    label: "山岳地域の暖房需要",
    detail:
      "軽井沢・志賀高原・蓼科・白馬等の標高1,000m超地域では年間平均気温10℃以下で、冬季最低気温-20℃級。暖房度日4,500超。観光業・別荘地で冬季電気代が他季の3〜4倍に達する事業者多数。",
  },
  {
    label: "観光業のシーズン需要変動",
    detail:
...
const industryProfile = [
  ...
  },
  {
    ...
    detail:
      "りんご・ぶどう・ワイン醸造（小布施ワイナリー・千曲川ワインバレー等）。CA冷蔵庫・発酵タンク・地下貯蔵庫の電力負荷。年間使用量50〜500万kWh規模。",
  },
  {
    label: "農業・酒造業・伝統産業",
    detail:
...
const energySaving = [
  ...
  },
  {
    ...
    detail:
      "長野県は晴天日数が多く、年間日照時間2,000時間超。屋根面積1,500m²以上の事業所で300kW級導入が現実的。標高高い地域は冬季発電量が確保しやすく、投資回収 7〜10年（補助金後 4〜6年）。",
  },
  {
...
const faqItems = [
  { question: "長野県の法人電気代水準は全国比どれくらいですか？", answer: "長野県内は中部電力エリアと東京電力エリアに分かれます。高圧電力（業務用）の単価について、全国的な水準との差は契約区分・使用量・負荷率により変わります。拠点ごとに供給エリアが異なる場合があるため、請求書から消費税と再エネ賦課金を除いた金額を使用量で割った実質単価で、拠点別に確認してください。" },
  ...
  { question: "長野県の電気代の平均・相場はいくらですか？", answer: "県単位で法人の電気代平均を示した公的統計はないため、契約区分ごとの全国確定単価を基準にして読みます。電力取引報の最新確定分である2026年5月分（全国計・販売額÷販売電力量）は、特別高圧17.45円/kWh、高圧22.78円/kWh、低圧電灯27.85円/kWh、低圧電力34.42円/kWhでした。この販売額には基本料金・電力量料金・燃料費調整額が含まれ、消費税・再エネ賦課金は含まれません。長野県は県内に中部電力エリアと東京電力エリアの境界が走るため、県全体を一つの単価でくくること自体が実態に合いません。事業所ごとに供給エリアを確認したうえで、自社の契約区分に対応する全国値と請求書の実質単価を比べてください。" },
      ...
      <main className="mx-auto min-h-screen w-full max-w-[1600px] bg-white px-4 py-8 text-slate-800 sm:px-6 lg:px-8">
        ...
        <section className="mt-6 space-y-6">
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-5">
            ...
            <p className="mt-3 text-sm leading-7 text-slate-700 sm:text-base">
              県内は中部電力エリアと東京電力エリアに分かれます。全国的な水準との差は契約区分・使用量・負荷率により変わります。県内に2エリアの境界があるため、実質単価（電力量料金+燃料費調整額+再エネ賦課金）ベースで両エリアを比較することが重要です。
            </p>
            <p className="mt-2 text-xs text-slate-500">
              ※ 本記事は中立的な情報整理を目的としており、特定の電力会社・契約形態を推奨するものではありません。
            </p>
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-5">
            ...
            <p className="mt-2 text-xs text-slate-500">
              ※ 上記4区分の数値は全国計・販売額÷販売電力量で求めた値であり、長野県または特定エリアの平均値ではありません。販売額には基本料金・電力量料金・燃料費調整額を含み、消費税・再エネ賦課金は含みません。出典: 電力・ガス取引監視等委員会「電力取引報」から算出（2026年5月分・確定）。単価の定義は{" "}
