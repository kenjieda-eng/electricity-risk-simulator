...
const overview = [
  ...
  },
  {
    ...
    detail:
      "当サイトの『沖縄エリアの法人電気代事情』は、エリア全体の市況・新電力動向（沖縄エリアは新電力の参入が限定的な傾向）・JEPX沖縄エリアの取扱いなどの『面』の情報を扱います。一方、本ページはおきでんという『特定企業』の法人向けプラン体系・燃料費調整額の算定方式・契約手続き・サポート体制という『個別企業のサービス詳細』を解説します。エリア全体の市況は地域一般記事を、特定企業のサービス詳細は本記事を、と読み分けてください。",
  },
  {
...
const procedures = [
  {
    ...
    detail:
      "法人の電力契約は、供給地点特定番号（22桁）・契約電力・使用実績データ（直近12ヶ月）を準備のうえ、見積依頼→条件提示→契約申込→供給開始という流れが一般的です。沖縄エリアは新電力の参入が限定的な傾向があるため、利用可能な事業者の範囲で相見積を取得し、契約期間・解約条件・燃調条件を契約書で確認することが重要です。",
  },
  {
    label: "供給地点特定番号の確認",
    detail:
...
const faqItems = [
  ...
  },
  {
    ...
    answer:
      "当サイトは特定企業の優劣を評価する立場にありません。一般社団法人エネルギー情報センターは特定企業を推奨も否定もしない中立的立場で判断材料を整理しています。沖縄エリアは新電力の参入が限定的な傾向があり、利用可能な事業者の範囲も本土と異なる場合があります。利用可能な範囲で相見積を取得し、自社条件に最適な選択をすることを推奨します。",
  },
  {
