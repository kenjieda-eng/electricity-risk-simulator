...
const pageTitle =
  "東京都の法人電気料金ガイド｜首都経済集積・データセンター・大型商業施設の契約最適化";
...
const electricSituation = [
  {
    ...
    detail:
      "東京都は東京電力パワーグリッド（送配電）・東京電力エナジーパートナー（小売）の中核エリア。23区と多摩地区で配電網構造が異なる。都内総電力需要は約780億kWhで東電エリア全体の30%弱を占める。系統規模は大きく、需給安定性は高水準。",
  },
  {
  ...
  },
  {
    ...
    detail:
      "23区はヒートアイランド現象で夏季最高気温が周辺県より2〜4℃高い。年間冷房度日（CDD24）は1,500以上で高い水準。冬季は比較的温暖で暖房度日2,000程度。冷房需要のピーク（7〜9月午後）が需給管理上最大の課題。",
  },
  {
...
const utilitiesList = [
  ...
  },
  {
    ...
    role: "全国系新電力",
    detail:
      "都内の新電力の一角。固定単価・市場連動の両方を提供。23区オフィスビル・データセンター・商業施設で実績多数。2022〜2023年の市場高騰局面で一部営業休止もあったが、2024年以降は再開。",
  },
  {
...
const switchingReality = [
  {
    label: "新電力切替の都内浸透度",
    detail:
      "2024年時点で都内法人の新電力シェアは30〜35%（経産省統計）と高い水準。23区都心では大規模事業者の競争入札による切替が進む。多摩地区中小事業者でも切替が拡大。",
  },
  {
    label: "市場連動プランのリスク認識",
    detail:
...
const faqItems = [
  { question: "東京都の法人電気代水準は全国比どれくらいですか？", answer: "高圧電力（業務用）の単価について、全国的な水準との差は契約区分・使用量・負荷率により変わるため、請求書から消費税と再エネ賦課金を除いた金額を使用量で割った実質単価で確認してください。23区の大規模事業者やデータセンターでは供給事業者の選択肢が多く、複数社から同一条件で見積を取って比較しやすい環境にあります。" },
      ...
      <main className="mx-auto min-h-screen w-full max-w-[1600px] bg-white px-4 py-8 text-slate-800 sm:px-6 lg:px-8">
        ...
        <section className="mt-6 space-y-6">
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-5">
            ...
            <p className="mt-3 text-sm leading-7 text-slate-700 sm:text-base">
              東京電力エリアの単価について、全国的な水準との差は契約区分・使用量・負荷率により変わります。供給事業者の選択肢が多いエリアであり、複数社から同一条件で見積を取って比較しやすい環境にあります。実質単価（電力量料金+燃料費調整額+再エネ賦課金）ベースで比較することが重要です。
            </p>
            <p className="mt-2 text-xs text-slate-500">
              ※ 本記事は中立的な情報整理を目的としており、特定の電力会社・契約形態を推奨するものではありません。
            </p>
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-5">
            ...
            <p className="mt-3 text-sm leading-7 text-slate-700 sm:text-base">
              東京都では国補助（SII等）に加え、都独自補助、23区補助、ZEB・ZEH支援事業が組合せ可能です。都内では補助金体系が充実しており、設備投資のタイミングを補助金スケジュールに合わせることで投資回収を1〜2年短縮できます。
            </p>
            <div className="mt-4 space-y-3">
              {subsidies.map((item) => (
                <div key={item.name} className="rounded-lg border border-slate-200 bg-slate-50 p-4">
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-5">
            ...
            <p className="mt-3 text-sm leading-7 text-slate-700 sm:text-base">
              東京都の新電力シェアは2024年時点で30〜35%程度と高い水準。23区都心では大規模事業者の競争入札による切替が標準化、需要家主導型PPAも急速に普及しています。
            </p>
            <p className="mt-2 text-xs text-slate-500">
              ※ 本記事は中立的な情報整理を目的としており、特定の電力会社・契約形態を推奨するものではありません。
            </p>
          ...
          <RelatedLinks
            ...
            links={[
              ...
              { href: "/chiyoda-ku-business-electricity-cost", title: "千代田区の法人電気料金ガイド", description: "大手町・丸の内・霞が関の大企業本社・官公庁の契約最適化。" },
              { href: "/koto-ku-business-electricity-cost", title: "江東区の法人電気料金ガイド", description: "湾岸物流・データセンター集積地・豊洲再開発の電力事情。" },
              { href: "/ota-ku-business-electricity-cost", title: "大田区の法人電気料金ガイド", description: "町工場集積・羽田空港物流・京浜工業地帯の契約最適化。" },
