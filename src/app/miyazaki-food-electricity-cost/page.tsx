...
const pageDescription =
  "宮崎県の食品製造業に特化。畜産加工（食肉）・本格焼酎・冷凍食品の集積を背景に、冷凍冷蔵・加熱殺菌・蒸留の電力プロファイル、九州電力エリアの単価事情（原発稼働＋太陽光導入量が多く昼間の出力制御）、契約最適化、補助金・PPA活用を実務目線で整理します。";
...
const electricSituation = [
  {
    ...
    detail:
      "宮崎県は肉用牛・豚・ブロイラー（鶏肉）の生産が盛んで、その川下にあたる食肉加工・冷凍・惣菜・飼料の工場群が都城市・宮崎市・日向市・延岡市・小林市などに集積しています。畜産加工を起点に、冷凍野菜・冷凍食品・チルド惣菜・調味料・本格焼酎（芋・麦）の蒸留所まで、食品製造業の裾野が広いのが県の特徴です。受託加工・OEMを含めれば県内の関連事業所数は相当規模に上ります。本ページは特定の電力会社・契約形態を推奨するものではありませんが、こうした集積構造を前提に九州電力エリア固有の論点を整理します（出典: 業界団体・経産省/エネ庁統計・自治体統計から整理）。",
  },
  {
  ...
  },
  {
    ...
    detail:
      "宮崎県は九州電力エリアに属し、小売は九州電力（本体が法人小売を担う）、送配電は九州電力送配電が担います。九州エリアは玄海・川内の原子力発電所（計4基）が稼働しているうえ、太陽光の導入量が多く、晴天の昼間には再エネ出力制御（出力抑制）が頻発するのが大きな特徴です。このため燃料費調整額の感応度は相対的に低めとされ、昼間の自家消費太陽光メリットが出やすい一方、系統側の出力制御の影響に留意する必要があります。JEPX九州エリア価格を参照しつつ（昼間に低下しやすい）、エリア固有の単価事情を踏まえた契約戦略が求められます（出典: 九州電力送配電 供給・系統情報／エネ庁から整理）。",
  },
];
...
const utilitiesList = [
  {
    ...
    role: "一般小売事業者（旧一電）",
    detail:
      "宮崎県内最大シェア。都城・宮崎市・日向・延岡の高圧・特別高圧の食品工場の長期需要家を多数抱えます。法人向けの高圧・特別高圧メニューが整備され、固定単価型・燃料費調整連動型ともに用意されています。九州は原発稼働と太陽光導入量が多い電源構成を背景に、燃料費調整額の感応度が相対的に低めとされる特徴があります。長期安定供給と災害復旧体制の優位性から大手取引は維持基調ですが、本記載は事実関係の整理を目的としたものです（出典: 九州電力 法人向け料金プランから整理）。本セクションは特定の電力会社・契約形態を推奨するものではありません。",
  },
  {
  ...
  },
  {
    ...
    role: "再エネ特化型／PPA",
    detail:
      "九州は太陽光の導入量が多く、晴天の昼間に系統側で出力制御が起きるほど再エネが豊富です。この環境下では、屋根オンサイトPPA（敷地・屋根面積を活かす自家消費）が昼間の冷凍負荷と相性がよく、メリットが出やすい点が宮崎×食品固有の論点です。オフサイトPPA（県内・九州圏の太陽光案件）やコーポレートPPAの引合いも拡大傾向です。導入可否は屋根面積・契約期間・系統条件・出力制御の影響で変わり、画一的な調達形態を勧める趣旨ではありません。",
  },
  {
  ...
  },
  {
    ...
    role: "市場参照",
    detail:
      "JEPX九州エリアのスポット価格は、太陽光導入量が多い供給構造を背景に、晴天の昼間に大きく低下しやすい（時にゼロ近傍となる）のが特徴です。一方で太陽光が落ちる夕方や曇天・需給逼迫時には価格が上昇することもあり、市場連動型契約では時間帯による変動リスクが残ります。2022〜2023年には市場連動採用の工場でも単価上昇を経験し、現在は固定回帰の動きと、昼間の自家消費太陽光を組合せる動きの両方が見られます。出典: 新電力ネット（https://pps-net.org/unit）を加工して整理。本記載は市場動向の整理を目的としたものです。",
  },
];
...
const priceBenchmark = [
  {
    ...
    detail:
      "大型食肉加工・冷凍工場（2,000kW超）の特別高圧電力量料金は、標準メニューで概ね14〜17円/kWh＋調整項目とされます。九州は原発稼働と太陽光導入量が多い電源構成を背景に、燃料費調整額の感応度が相対的に低めとされる特徴があります。燃料費調整額（九州はエリア特性として感応度が相対的に低め）と再エネ賦課金（2026年度4.18円/kWh・確定）を加算した実質単価は19〜24円/kWhレンジが目安です。数値は目安であり、実際の単価は契約条件・新電力選定で変動します（出典: 業界団体・経産省/エネ庁統計から整理）。",
  },
  {
  ...
  },
  {
    ...
    detail:
      "九州電力エリアは玄海・川内の原発（計4基）が稼働し、太陽光の導入量が多いため、火力（LNG・石炭）への依存度が他エリアより相対的に低く、為替（円安）や燃料スポット価格の変動に対する燃料費調整額の感応度が相対的に低めとされるのがエリア固有の特徴です。2022〜2023年の全国的な燃料高騰局面でも、九州の燃調変動幅は火力依存度の高いエリアと比べて穏やかに推移した側面があります。とはいえ原発の定期検査や悪天候時には火力比率が上がるため、燃調がゼロではない点に留意が必要です（出典: 九州電力 単価実績・エネ庁エネルギー白書から整理）。",
  },
  {
    label: "再エネ賦課金の累積負担",
    detail:
...
const costFactors = [
  ...
  },
  {
    ...
    detail:
      "九州電力エリアは原発稼働と太陽光導入量が多いことにより火力依存度が相対的に低いため、燃料費調整額の感応度が他エリアより低めとされるのがエリア固有の特徴です。これは燃料高騰局面で単価が急騰しにくいメリットになり得る一方、晴天の昼間は再エネ出力制御（出力抑制）が頻発するため、系統への売電や系統側の事情に左右される面があります。宮崎の食品工場では、この相対的に安定した単価環境と昼間の自家消費太陽光メリットを前提に固定vs市場連動の選択を検討するのが実務的です。どちらが適するかは使用パターン次第で一概には言えません。",
  },
  {
...
const industryProfile = [
  {
    ...
    detail:
      "都城市は肉用牛・豚・ブロイラーの畜産が盛んで、その川下にあたる食肉加工・冷凍・飼料の事業所が集中するエリアです。急速凍結機・大型冷凍冷蔵倉庫を抱える高圧・特別高圧の食品工場が多く、冷凍負荷を含む恒常負荷が大きい点が共通します。加熱殺菌の蒸気需要もあり、契約電力最適化と冷凍機最適化の余地があります。",
  },
  {
  ...
  },
  {
    ...
    detail:
      "宮崎の食品産業は盛んな畜産（肉用牛・豚・ブロイラー）を起源に、食肉加工・冷凍・惣菜・飼料・本格焼酎までの裾野が県内で完結しやすい産業基盤の上に成り立っています。原料調達から加工・冷凍保管・物流までのエコシステムを形成し、これらの事業所群は九州電力エリアの原発稼働＋太陽光という電源構成のもとで電力を調達しています。",
  },
];
...
const faqItems = [
  {
    ...
    answer:
      "九州電力エリアは玄海・川内の原発（計4基）が稼働し、太陽光の導入量が多いため、火力依存度が相対的に低く、燃料費調整額の感応度も相対的に低めとされるのがエリア固有の特徴です。燃料高騰局面で単価が急騰しにくいメリットがある一方、晴天の昼間は再エネ出力制御（出力抑制）が頻発し、JEPX九州エリア価格が昼間に低下しやすいため、自家消費太陽光のメリットが出やすい点も特徴です。ただし原発の定期検査や悪天候時には火力比率が上がるため燃調がゼロになるわけではありません。なお本回答は特定の電力会社・契約形態を推奨するものではありません（出典: 九州電力 単価実績・エネ庁から整理）。",
  },
  {
  ...
  },
  {
    ...
    answer:
      "屋根面積を確保できる工場では現実的な選択肢になり得ます。九州は太陽光の導入量が多く昼間の発電量が大きく、食品工場の昼間の冷凍負荷と自家消費の相性がよいのが利点です。初期投資ゼロでPPA事業者が設備を所有し、自社は一定期間の電力購入契約を結ぶ形が標準で、再エネ調達と電気代単価下げの両立が期待できます。ただし晴天の昼間は系統側の出力制御が起きるほど再エネが豊富なため、売電よりも自家消費を主体に設計するのが実務的です。導入可否は屋根面積・契約期間・系統条件・建屋構造で変わるため、複数事業者の試算比較が前提となります。本回答は一般的な整理であり、個別案件の成立を保証するものではありません。",
  },
  {
      ...
      <main className="mx-auto min-h-screen w-full max-w-[1600px] bg-white px-4 py-8 text-slate-800 sm:px-6 lg:px-8">
        ...
        <header className="mt-4 rounded-xl border border-sky-200 bg-sky-50 p-6">
          ...
          <p className="mt-4 text-sm leading-7 text-slate-700 sm:text-base">
            宮崎県は肉用牛・豚・ブロイラーの畜産が盛んで、食肉加工・冷凍・惣菜・本格焼酎（芋・麦）まで食品製造業の裾野が広い地域です。本ページでは「宮崎県 × 食品製造業」というクロス領域に絞り、九州電力エリア固有の単価事情（原発稼働＋太陽光導入量が多く晴天の昼間は出力制御が頻発）と、冷凍冷蔵／急速凍結／加熱殺菌・蒸留の電力プロファイル、契約最適化、補助金・PPA活用までを実務目線で整理します。なお本ページは特定の電力会社・契約形態を推奨するものではありません。
          </p>
        ...
        <section className="mt-6 space-y-6">
          <section className="rounded-xl border border-sky-200 bg-sky-50 p-5">
            ...
            <p className="mt-3 text-sm leading-7 text-slate-700 sm:text-base">
              宮崎県は肉用牛・豚・ブロイラーの畜産が盛んで、その川下にあたる食肉加工・冷凍・惣菜・飼料・本格焼酎（芋・麦）の工場群が都城・宮崎市・日向・延岡・小林等に広がり、急速凍結機・大型冷凍冷蔵倉庫を抱える高圧・特別高圧の食品工場が多数立地しています。冷凍冷蔵の恒常負荷が大きい点が県内食品工場の電力構造の共通項です。
            </p>
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-5">
            ...
            <p className="mt-3 text-sm leading-7 text-slate-700 sm:text-base">
              特別高圧・高圧の単価レンジ、燃料費調整額の感応度（九州は相対的に低め）、再エネ賦課金の累積負担を、食品工場の代表的な契約タイプ別に整理します。九州固有の原発稼働・豊富な太陽光・燃調低感応の特性を踏まえた契約戦略が経営判断の中心となります。
            </p>
            <div className="mt-4 space-y-3">
          ...
          <RelatedLinks
            ...
            links={[
              ...
              { href: "/region-kyushu-business-electricity", title: "九州電力エリアの法人電気代事情", description: "九州エリアの料金体系・原発稼働・太陽光導入量の多さ。" },
