...
export default function LowVoltagePowerRetrospectiveCharts() {
  const yearlySummary = useMemo(() => getYearlySummary(), []);

  const monthlyLineData = useMemo<ChartData<"line">>(
    () => ({
      labels: LOW_VOLTAGE_POWER_MONTHLY_PRICES.map((point) => monthLabel(point.date)),
      datasets: [
        {
          label: "低圧電力 単価（円/kWh）",
          data: LOW_VOLTAGE_POWER_MONTHLY_PRICES.map((point) => point.price),
          borderColor: "#0f766e",
  ...
  return (
    <section className="mt-8 rounded-xl border border-slate-200 bg-white p-5">
      ...
      <p className="mt-2 text-sm leading-7 text-slate-600 sm:text-base">
        月次データの折れ線と、年平均の棒グラフで低圧電力単価の推移を可視化しています。2022年以降の急変幅が大きく、
        低圧電灯より振れ幅が出やすい傾向が確認できます。
      </p>
