...
export default function RetrospectiveCharts() {
  const yearlySummary = useMemo(() => getYearlySummary(), []);

  const monthlyLineData = useMemo<ChartData<"line">>(
    () => ({
      labels: EXTRA_HIGH_VOLTAGE_MONTHLY_PRICES.map((point) => monthLabel(point.date)),
      datasets: [
        {
          label: "特別高圧 単価（円/kWh）",
          data: EXTRA_HIGH_VOLTAGE_MONTHLY_PRICES.map((point) => point.price),
          borderColor: "#0f766e",
  ...
  return (
    <section className="mt-8 rounded-xl border border-slate-200 bg-white p-5">
      ...
      <p className="mt-2 text-sm leading-7 text-slate-600 sm:text-base">
        月次データの折れ線と、年平均の棒グラフで推移を可視化しています。2022年から2023年にかけての急上昇と、
        2024年以降の緩やかな低下がひと目で確認できます。
      </p>
