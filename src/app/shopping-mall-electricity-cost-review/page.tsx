        ...
        <section className="rounded-xl border border-slate-200 bg-white p-5">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            ...
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
              ...
              <p className="mt-1 text-sm leading-6 text-slate-600">
                ピーク時間帯の電力需要を<Link href="/battery-suited-corporations" className="text-sky-700 underline underline-offset-2 hover:text-sky-900">蓄電池</Link>で補完しデマンド値を抑制することで、<Link href="/basic-charge-explained" className="text-sky-700 underline underline-offset-2 hover:text-sky-900">基本料金</Link>の削減につながります。大容量システムの導入は専門的な設計と費用対効果分析が必要です。
              </p>
            </div>
            <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
              <p className="text-sm font-semibold text-slate-900">屋根・駐車場への太陽光発電</p>
              <p className="mt-1 text-sm leading-6 text-slate-600">
                広大な屋根や駐車場（カーポート型）への<Link href="/self-consumption-solar-cost-benefit" className="text-sky-700 underline underline-offset-2 hover:text-sky-900">太陽光</Link>発電設置は、昼間の電力自給に有効です。PPA（電力購入契約）モデルでは初期費用なしでの導入も可能です。
              </p>
            </div>
          </div>
        </section>
