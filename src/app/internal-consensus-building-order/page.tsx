...
const howToSteps = steps.map((s) => ({ name: s.step, text: s.content }));

export default function InternalConsensusBuildingOrderPage() {
  return (
    <>
      <ArticleJsonLd
        headline="契約見直しの社内合意を進める順番｜関係者の巻き込み方と段取り"
        description="電力契約見直しの社内合意を効率的に進めるための順番と段取りを解説。関係者の特定、情報収集のタイミング、説明の順番、稟議提出までのスケジュール管理まで実務目線でまとめます。"
        url="https://simulator.eic-jp.org/internal-consensus-building-order"
    ...
    <main className="mx-auto min-h-screen w-full max-w-[1600px] bg-white px-4 py-8 text-slate-800 sm:px-6 lg:px-8">
      <nav aria-label="パンくず" className="text-sm text-slate-600">
        ...
        <span className="px-2">›</span>
        <span className="text-slate-800">社内合意を進める順番</span>
      </nav>
