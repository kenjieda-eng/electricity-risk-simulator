...
const electricSituation = [
  ...
  },
  {
    ...
    detail:
      "千葉県は東京湾岸＝東京電力エリアに属し、小売は東京電力エナジーパートナー（東電EP）、送配電は東京電力パワーグリッドが担います。東京エリアは電源としてLNG・石炭火力への依存度が高く、需要規模も大きいエリアです。火力依存度が高いことから、為替（円安）や燃料スポット価格の変動に対する燃料費調整額の感応度が相対的に中〜高で、高騰局面での単価の上振れ幅が大きい点がエリア固有の特徴です。JEPX東京エリア価格を参照しつつ、燃調感応度が中〜高というエリア事情を踏まえた契約戦略（固定／市場連動の選択）が求められます（出典: 業界団体・経産省/エネ庁統計・自治体統計から整理）。",
  },
];
...
const utilitiesList = [
  {
    ...
    role: "一般小売事業者（旧一電）",
    detail:
      "千葉県内最大シェア。市原・袖ケ浦・千葉・市川の京葉コンビナートをはじめ、特別高圧・高圧の化学・石油化学の長期需要家を多数抱えます。法人向けの特別高圧・高圧メニューが整備され、固定単価型・燃料費調整連動型ともに用意されています。東京エリアはLNG・石炭火力依存度が高く需要規模が大きく、燃料費調整額の感応度が相対的に中〜高とされる特徴があります。長期安定供給と大口需要家向けの体制から大手取引は維持基調ですが、本記載は事実関係の整理を目的としたものです（出典: 業界団体・経産省/エネ庁統計・自治体統計から整理）。",
  },
  {
  ...
  },
  {
    ...
    role: "市場参照",
    detail:
      "JEPX東京エリアのスポット価格は、需要規模が大きくLNG・石炭火力依存度が高い供給構造を背景に、燃料スポット価格や気温による需給で変動しやすく、高騰局面では他エリアより上振れ幅が大きくなる傾向があります。市場連動型契約では変動リスクが残り、2022〜2023年には市場連動採用の工場でも単価上昇を経験し、現在は固定回帰の動きが見られます。出典: 新電力ネット（https://pps-net.org/unit）を加工して整理。本記載は市場動向の整理を目的としたものです。",
  },
];
...
const priceBenchmark = [
  ...
  },
  {
    ...
    detail:
      "東京電力エリアはLNG・石炭火力への依存度が高く、需要規模が大きいため、為替（円安）や燃料スポット価格の変動に対する燃料費調整額の感応度が相対的に中〜高とされるのがエリア固有の特徴です。2022〜2023年の全国的な燃料高騰局面では、東京の燃調変動幅は火力依存度が低いエリアと比べて大きく、高騰時の上振れ幅が大きくなりました。連続運転で使用量が大きい石油化学では、燃調の上振れが経営に与える影響が大きいため、固定／市場連動の選択が重要です（出典: 業界団体・エネ庁エネルギー白書から整理）。",
  },
  {
    label: "再エネ賦課金の累積負担",
    detail:
...
const costFactors = [
  ...
  },
  {
    ...
    detail:
      "東京電力エリアはLNG・石炭火力への依存度が高く需要規模が大きいため、燃料費調整額の感応度は相対的に中〜高とされるのがエリア固有の特徴です。これは燃料高騰局面で単価が上振れしやすいというリスクであり、連続運転で使用量が大きい石油化学では燃調の上振れが経営に与える影響が大きくなります。固定vs市場連動の選択が単価の安定性に直結するため、この相対的に高い燃調感応度を前提に契約を検討するのが実務的です。どちらが適するかは使用パターン・リスク許容度次第で一概には言えません。",
  },
  {
...
const switchingReality = [
  {
    label: "東京エリアの新電力浸透度",
    detail:
      "東京電力エリアの新電力比率は、需要規模が大きく市場流動性も高いことを背景に、他エリアと比べて新電力の参入余地が大きい傾向があるとされます（出典: 業界団体・経産省/エネ庁統計から整理）。一方で燃調感応度が中〜高で高騰時の上振れが大きいため、切替メリットは燃調条件・契約期間・再エネ付加価値で総合的に判断する必要があります。年間使用量の大きい化学・石油化学では競争入札による相見積が有効ですが、超大口は供給枠の確保が前提となり、最終判断は自社の使用実態に即して行う必要があります。",
  },
  {
  ...
  },
  {
    ...
    detail:
      "メリット: 大口需要家向けエネルギーマネジメント支援・大規模な供給体制・災害時復旧体制。デメリット: 新電力との比較で単価がやや高めになる局面、燃料費調整額の条件差。東京は燃調感応度が中〜高で高騰時の上振れが大きいため、継続か切替かは燃調条件を含めた総合的な比較が必要です。いずれにせよ本記載は特定の電力会社を推奨するものではありません。",
  },
  {
...
const faqItems = [
  {
    ...
    answer:
      "東京電力エリアはLNG・石炭火力への依存度が高く、需要規模が大きいとされるのがエリア固有の特徴です。火力依存度が高いため燃料費調整額の感応度が相対的に中〜高で、燃料高騰局面で単価が上振れしやすい点に留意が必要です。これは連続運転で使用量が大きい石油化学にとって、燃調の上振れが経営に与える影響が大きいことを意味します。一方で需要規模が大きく市場流動性も高いため新電力の選択肢は広く、固定／市場連動の選択が単価の安定性に直結します。なお本回答は特定の電力会社・契約形態を推奨するものではありません（出典: 業界団体・経産省/エネ庁統計から整理）。",
  },
  {
      ...
      <main className="mx-auto min-h-screen w-full max-w-[1600px] bg-white px-4 py-8 text-slate-800 sm:px-6 lg:px-8">
        ...
        <header className="mt-4 rounded-xl border border-sky-200 bg-sky-50 p-6">
          ...
          <p className="mt-4 text-sm leading-7 text-slate-700 sm:text-base">
            千葉県は市原市・袖ケ浦市・千葉市・市川を結ぶ京葉臨海コンビナートに、石油精製・石油化学（エチレンプラント・誘導品・基礎化学品）の大規模な集積を擁する重化学工業地帯です。本ページでは「千葉県 × 化学工業」というクロス領域に絞り、東京電力エリア固有の単価事情（LNG・石炭依存で燃調感応度が中〜高）と、石油化学プラントの電力プロファイル（連続運転で停止不可のベース負荷が極大、コージェネ自家発と大口買電の併存）／大型コンプレッサー・蒸留・電解の連続負荷、契約最適化、補助金・PPA活用までを実務目線で整理します。なお本ページは特定の電力会社・契約形態を推奨するものではありません。
          </p>
