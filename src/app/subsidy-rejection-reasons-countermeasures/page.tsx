...
const overview = [
  ...
  },
  {
    ...
    detail:
      "内容以前に、必要書類の不足・記載漏れ・要件非該当・期限超過といった形式不備で失格になるケースも少なくありません。これは内容を磨く以前の問題で、提出前のチェックリストとダブルチェックで防ぎやすくなります。形式不備での失格は最も避けやすい不採択です。",
  },
];
...
const faqItems = [
  ...
  },
  {
    ...
    answer:
      "形式不備（必要書類の不足・記載漏れ・要件非該当・期限超過）は内容審査の前に失格となる、最も避けやすい不採択です。対策は、公募要領に記載された必要書類・記載事項・対象要件・提出期限を一覧化したチェックリストを作り、提出前に複数人でダブルチェックすることです。特に要件への該当（事業規模・設備区分・対象経費等）は思い込みで判断せず公募要領で確認します。形式不備は内容の良し悪し以前の問題なので、チェック体制を組めば防ぎやすくなります（出典: 各補助金事務局／2025年度時点）。",
  },
  {
