 ...
 *   c. 単価リテラルを「同一行で最も近い年度参照 /(20[1-3]\d)年度/」と対にし、
 *      SSOT 値と比較 → 不一致なら検出。年度に対応できなければ「年度不明・要目視」へ。
 *      （年度レンジは 2010-2039。SSOTは2012年度から収録のため、2012年度の0.22円/kWh等の
 *        早期年度を後続年度に誤ペアさせないよう 2020- ではなく 2010- まで認識する。）
 *   d. 除外:
 *      - 価格が属する「文」（句点区切り）に「燃料費調整」または「燃調」を含む（文スコープ。
 *        旧・窓スコープだと隣接文の賦課金単価まで巻き込みマスクするため文単位に限定）
 *      - 単価の直前局所文脈（直前15文字程度）に「容量拠出金 / 平均単価 / 託送料金」がある
 *        （＝再エネ賦課金でない別制度コスト/単価。per-price 局所判定のため同一文内の本物の
 *          賦課金ドリフトは巻き込まない）
...
// 年度参照: 2010〜2039年度（SSOTは2012年度から収録。2012年度0.22円/kWh等の早期年度を
// 後続年度へ誤ペアさせないため 2020- ではなく 2010- まで認識する）。
const YEAR_RE = /(20[1-3]\d)年度/g;
const YEAR_TEST_RE = /20[1-3]\d年度/;
...
async function scanFile(file, ssot) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const pm of priceMatches) {
      ...
      // d. 非賦課金の単価ラベル（容量拠出金・平均単価・託送料金）が単価の直前局所文脈にある場合は
      //    その単価のみ除外（per-price 局所判定。直前15文字程度。同一文内の本物の賦課金ドリフトは
      //    巻き込まない＝窓/文一括除外にしない）。
      const labelCtx = line.slice(Math.max(0, start - 15), start);
      if (/容量拠出金|平均単価|託送料金/.test(labelCtx)) continue;
